  ...
  Message.watch((message) => {
    ...
    if (message.conversationType == conversationType && message.targetId === targetId) {
      ...
      Conversation.clearUnreadCount({
        ...
        console.log("Conversation.wather", Conversation.wather);
        Conversation.wather.notify(conversationList);
      });
    }
  });
};
...
const sendImage =  (context) => {
  my.chooseImage({
    ...
    success: (res) => {
      let tempFilePath = res.apFilePaths[0];
      my.getImageInfo({
        src: tempFilePath,
        success: (res) => {
        ...
        fail: (err) => {
          console.log('wx.getImgaeInfo', err)
        } 
      })
    }
  })
...
const sendFile = (context) => {
  ...
  Message.sendFile({
    conversationType,
    targetId,
    name: "renderjstest.zip",
    size: 94800,
    fileUrl:
      "https://gz.bcebos.com/v1/rongcloud-file/4__RC-2020-8-13_499_1597289279033692537d7-f9bd-46b5-a014-d97bcbcb3e5c.zip",
  }).then((msg) => {
...
const playMusic = (context, event) => {
  let newMusicComponent = event;
  let { playingMusicComponent, messageList } = context.data;
  let { props: { message: { messageUId: newPlayId } } } = newMusicComponent

  let playingId = '';

  // 连续点击播放不同音乐
  if (playingMusicComponent) {
    let { props: { message } } = playingMusicComponent;
...
const stopMusic = (context, event) => {
  ...
  let { props: { message: { messageUId } } } = musicComponent;
...
Page({
  ...
  onReady() {
    my.createSelectorQuery().select('#rong-message-wrap-el').boundingClientRect().
    select('#rong-messages').boundingClientRect().exec((res) => {
      let innerHeigth = res[0].height;
      let outHeight = res[1].height;
      let scrollTop = innerHeigth - outHeight;
      my.pageScrollTo({
        scrollTop: scrollTop + 'px',
        duration: 300,
        selector: '#rong-message-wrap-el'
      });
    });
  }
})