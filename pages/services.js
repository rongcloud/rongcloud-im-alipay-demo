...
Message.getChatRoomMessageList = (params) => {
  ...
  return imInstance.getChatroomHistoryMessages(targetId, data).then(res => {
    // 获取聊天室历史信息成功
    if (res.code === 0) {
      let messageList = res.data.list;
...
const sortConList = (conversationList) => {
  ...
  const topConversationList = _sortListBySentTime(
    splitConversationList.topConversationList
  );
  const unToppedConversationList = _sortListBySentTime(
...
Status.connect = (user) => {
  ...
  imInstance.addEventListener(Events.CONVERSATION, (event) => {
...
const zfbUpload = (fileInfo, token) => {
...
File.upload = (fileInfo, uploadType) => {
  ...
  return imInstance.getFileToken(fileType, fileName).then(result => {
    ...
    return zfbUpload(fileInfo, token)
  }).then(res => {
